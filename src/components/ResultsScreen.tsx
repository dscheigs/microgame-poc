...
const ResultsScreen: React.FC<ResultsScreenProps> = ({ totalTime, levelResults, onRestart }) => {
  ...
  const completedLevels = levelResults.filter(result => result.completed).length;
        ...
        <div style={{ 
          ...
          {levelResults.map((result, index) => (
            ...
            >
              ...
              <span>
                {result.completed ? '✓ Completed' : result.skipped ? '⏭ Skipped' : '○ Not attempted'}
              </span>
            </div>
          ))}
        </div>
